    ...
    if (resendKey && candidateEmail) {
      const html = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
          ...
          <p style="color: #555; font-size: 16px; line-height: 1.6;">
            <strong>${employerName || "A company"}</strong> viewed your Hyr profile and wants to connect with you.
          </p>
          ${message ? `<div style="background: #f5f5f5; border-radius: 8px; padding: 16px; margin: 20px 0;"><p style="color: #333; font-size: 14px; margin: 0;"><strong>Their message:</strong></p><p style="color: #555; font-size: 14px; margin: 8px 0 0;">${message}</p></div>` : ""}
          <p style="color: #555; font-size: 14px;">
            ${employerEmail ? `You can reply directly to <strong>${employerEmail}</strong>` : "Log in to Hyr to see more details."}
          </p>
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: candidateEmail,
          subject: `${employerName || "A company"} is interested in your profile — Hyr`,
          html,
        }),
      })
    }
